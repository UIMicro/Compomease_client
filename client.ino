...
const double high1k_value = 989.77;
const double high100k_value = 100082;
const double low1k_value = 996.08;
const double low100k_value = 98994;
const double baseSense_value = 197800;
const double highSense_value = 987.84;
...
void measureResistor() {
    ...
    if (resistance > 1000) {
        ...
        tft->printf("%.*g Ohms", 4, resistance);
    }
...
void measureNPN() {
    ...
    if (resB->getPos() == 5 || resB->getPos() == X9C::MAX) {
        tft->drawRGBBitmap(0, 0, (const uint16_t*)gImage_s, 240, 240);
        return;
    }
    ...
    for (int curveCnt = 0; curveCnt < curveNum; curveCnt++) {
        ...
        for (auto i = X9C::MIN; i <= X9C::MAX; ++i) {
            resC->setPos(i);
            ics[curveCnt][i] =
                    repeatSampleDifferential(highInputSense, highInput, 64, 0) /
                    highSense_value;
            uce[curveCnt][i] = repeatSampleDifferential(highInput, low1k, 64, 0);
...
void measureDiode() {
    ...
    for (int i = X9C::MIN; i <= X9C::MAX; ++i) {
        resC->setPos(i);
        id[i] = repeatSampleDifferential(highInputSense, highInput, 160, 0) /
                        highSense_value;
        ud[i] = repeatSampleDifferential(highInput, low1k, 160, 0);
#ifdef withDisplay
        finishRatio = (i * 1.0) / (1.0 * X9C::MAX);
        if (true) {
    ...
    // compute diode parameters with least square method
    //  i=Is(exp(u/Ut)-1)
    //  approx. ln i = u/Ut + ln is
    debug = true;
    double* lni = new double[X9C::MAX + 1];
    double lni_avg = 0, u_avg = 0;
    int start_pos;
    bool started = false;
    for (int i = X9C::MIN; i <= X9C::MAX; ++i) {
      lni[i] = log(id[i] + 0.00001);
      if (!started && lni[i] > -10) {
        started = true;
        start_pos = i;
      }
    ...
    lni_avg /= (X9C::MAX - start_pos + 1);
    u_avg /= (X9C::MAX - start_pos + 1);
      if (debug) {
        Serial.printf("lni_avg: %lf\n", lni_avg);
        Serial.printf("u_avg: %lf\n", u_avg);
      }

    double tmp1 = 0, tmp2 = 0;
    for (int i = start_pos; i <= X9C::MAX; ++i) {
      tmp1 += (ud[i] - u_avg) * (lni[i] - lni_avg);
      tmp2 += (ud[i] - u_avg) * (ud[i] - u_avg);
    }
    double Ut_hat = tmp2 / tmp1;
    double Is_hat = exp(lni_avg - u_avg / Ut_hat);
    if (debug) {
      Serial.printf("Estimated Ut: %lf, Is: %lf", Ut_hat, Is_hat);
    }
    debug = false;
    ...
    tft->setTextColor(ST77XX_WHITE);
    tft->setCursor(20, 20);
    tft->printf("U 100mV/div");
    tft->setCursor(120, 200);
    tft->printf("I/mA");
    tft->setCursor(20, 50);
    tft->printf("Ut: %.*gmV", 4, Ut_hat * 1000.0);
    tft->setCursor(20, 70);
    tft->printf("Is: %.*gpA", 4, Is_hat * 1000000000000.0);
    tft->setCursor(20, 90);
    tft->printf("UD: %.*gmV", 4, ud[start_pos] * 1000.0);
...
void drawDynamicTriangle() {
    ...
    static const Vector2 points[] = {Vector2(0, 0), Vector2(0, 0),
                                                                     Vector2(240, 240), Vector2(240, 240)};
    static const Vector2 normals[] = {Vector2(1, 0), Vector2(0, 1),
                                                                        Vector2(-1, 0), Vector2(0, -1)};

    static Vector2 V[] = {Vector2(300, 200), Vector2(100, 205),
                                                Vector2(150, 100), Vector2(100, 200),
                                                Vector2(100, 210), Vector2(200, 100)};
    static Vector2 X[] = {Vector2(50, 50), Vector2(80, 50), Vector2(65, 75),
                                                Vector2(90, 90), Vector2(90, 30), Vector2(90, 60)};
    ...
    tft->fillRect((int16_t)(xmin - 7), (int16_t)(ymin - 7),
                                (int16_t)(xmax - xmin + 14), (int16_t)(ymax - ymin + 14),
                                ST77XX_BLACK);
    if (debug) {
        tft->setCursor(0, 0);
        tft->setTextColor(ST77XX_WHITE);
    ...
    for (int i = 0; i < edgeCount * 2; i += 2) {
        tft->drawLine((int16_t)X[E[i]][0], (int16_t)X[E[i]][1],
                                    (int16_t)X[E[i + 1]][0], (int16_t)X[E[i + 1]][1],
                                    ST77XX_WHITE);
    }
    xmin = 240;
    ymin = 240;
